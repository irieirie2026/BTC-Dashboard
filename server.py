...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path.startswith("/api/tradfi/"):
            parsed = urlparse(self.path)
            section = parsed.path[len("/api/tradfi/") :].strip("/")
        ...
        if self.path.startswith("/api/defi/"):
            section = self.path[len("/api/defi/") :].split("?")[0].strip("/")
            try:
                payload = get_defi_payload(section)
                body = json.dumps(payload).encode()
                self.send_response(200)
                self.send_header("Content-Type", "application/json")
                self.send_header("Access-Control-Allow-Origin", "*")
        ...
        if self.path.startswith("/api/macro/"):
            section = self.path[len("/api/macro/") :].split("?")[0].strip("/")
            try:
                payload = get_macro_payload(section)
                body = json.dumps(payload).encode()
                self.send_response(200)
                self.send_header("Content-Type", "application/json")
                self.send_header("Access-Control-Allow-Origin", "*")
        ...
        if self.path.startswith("/api/exchanges/"):
            section = self.path[len("/api/exchanges/") :].split("?")[0].strip("/")
            try:
                payload = get_exchanges_payload(section)
                body = json.dumps(payload).encode()
                self.send_response(200)
                self.send_header("Content-Type", "application/json")
                self.send_header("Access-Control-Allow-Origin", "*")
        ...
        if self.path.startswith("/api/news/"):
            section = self.path[len("/api/news/") :].split("?")[0].strip("/")
            try:
                payload = get_news_payload(section)
                body = json.dumps(payload).encode()
                self.send_response(200)
                self.send_header("Content-Type", "application/json")
                self.send_header("Access-Control-Allow-Origin", "*")
