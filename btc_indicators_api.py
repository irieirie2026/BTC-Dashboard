...
def get_snapshot_payload(*, refresh: bool = False) -> dict[str, Any]:
    ...
    cells: dict[str, dict] = {
        "rich_top100_pct": _cell(bitinfo.get("rich_top100_pct"), "BitInfoCharts", {"fetchedAt": bitinfo.get("fetchedAt")}),
        "rich_top1000_pct": _cell(bitinfo.get("rich_top1000_pct"), "BitInfoCharts", {"fetchedAt": bitinfo.get("fetchedAt")}),
        "wealth_top10_pct": _cell(bitinfo.get("wealth_top10_pct"), "BitInfoCharts", {"fetchedAt": bitinfo.get("fetchedAt")}),
        ...
        "open_interest": _cell(oi.get("value"), "Binance Futures", {"fetchedAt": oi.get("fetchedAt")}),
        "btc_dominance": _cell(dominance.get("value"), "CoinGecko", {"fetchedAt": dominance.get("fetchedAt")}),
    }
...
def get_distribution_payload(*, refresh: bool = False) -> dict[str, Any]:
    bitinfo = fetch_bitinfo_snapshot(refresh=refresh)
    cohorts = fetch_bitinfo_wallet_cohorts(refresh=refresh)
    return {
        "wealth": {
...
def get_series_payload(
    ...
    *,
    ...
    if indicator == "puell_multiple":
        data = compute_puell_multiple(refresh=refresh)
    ...
    if indicator in BGEOMETRICS_SERIES:
        ...
        data = fetch_bgeometrics_series(metric, refresh=refresh)
        return {"indicator": indicator, **data}
    ...
    if indicator in bc_map:
        data = fetch_blockchain_chart(bc_map[indicator], timespan, refresh=refresh)
        return {"indicator": indicator, **data}
    ...
    if indicator == "btc_dominance":
        dom = fetch_coingecko_dominance(refresh=refresh)
        return {
            "indicator": indicator,
